...
|me✌️         |[15188](https://github.com/helix-editor/helix/pull/15188)|`enable-siagnostics` option|
...
Scratch buffers are ignored in saving logic.
...
These characters are now also considered word characters: `"`, `'`, \`, `%`, `&`, `!`, `*`, `$`, `“`, `”` `‘`, `’`, `«`, `»`.
...
But *does* affect all the other ones: indentation, closest pair, class, function, parameter, comment, test, entry, xml-element, any other character (quotes `'`, `"`, \` mostly follow the following, but have slightly special behavior that will be explained at the end).
...
the new selection is **always** forced *backward*²
...
² indentation, closest pair, class, function, parameter, comment, test, entry, xml-element, any other character, **git change**. The git change textobject doesn't do the repeat feature but is still forced backward for better consistency.
...
As for quotes. Say you have `'something'` selected, and want to have `something` selected. In upstream, nothing would happen if you pressed <kbd>mi'</kbd>, but it *does* work in this fork. \
The *how* of it working is different from all the other textobjects explained above. Instead of moving left from the *start* of the selection, you move left from the *end* of the selection. \
With `'something'`, a cursor is created on `g`, and <kbd>mi'</kbd> runs from there.
You can think of it as you doing `ensure_selections_forward` (<kbd>shift+alt+;</kbd>) and then pressing <kbd>hmi'</kbd>. \
‼️This shenanigan only happens if the first <kbd>mi'</kbd> attempt doesn't change any of your selections. Same as with the other textobjects.
...
(except [harp](#harp-integration), which are explained in a later section)
