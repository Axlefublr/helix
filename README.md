...
On write, the contents of the buffer are piped into `helix-piper` (if you have it in `$PATH`; otherwise this step is skipped) and are replaced with its output. \
...
Due to <kbd>ctrl+v</kbd> now pasting, the hotkey to open the result in a vertial split is moved from <kbd>ctrl+v</kbd> to <kbd>ctrl+r</kbd>.
