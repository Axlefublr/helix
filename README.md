...
|@NikitaRevenco|[12208](https://github.com/helix-editor/helix/pull/12208)  |New command to open hover documentation in a new buffer instead of a popup|
|@EpocSquadron |[9843](https://github.com/helix-editor/helix/pull/9843)    |Implement new textobject for indentation level|
|@nik-rev      |[12308](https://github.com/helix-editor/helix/pull/12308)  |Color swatches (🟩 green 🟥 #ffaaaa )|
|@pantos9000   |[10576](https://github.com/helix-editor/helix/pull/10576)  |Add same_line and anchored movements|
|@intarga      |[9143](https://github.com/helix-editor/helix/pull/9143)    |Persistent state (⚠️ currently doesn't work; waiting on the author to rebase on master, to merge it in again)|
...
### Changed behavior of builtin commands

`:yank-join` now respects the `default-yank-register` option; previously it would place the result to `"` anyway. I pushed this change as a PR to upstream, hopefully it's gonna get accepted in less than 3 years.

`:yank-join` now understands literal string `space` to mean a space (`:yank-join space`). Currently there is no way to specify a space as an argument to a :command as far as I'm aware, so that's why I added this workaround.
...
`:buffer-delete-file` (with aliases `db`, `del`, `delete`) deletes the *current* buffer's real file, and also forcefully closes the buffer. You can think of it as `:sh rm %p` + `:buffer-close!` in a single command.

`:write-buffer-close-or-quit` does `:quit` if on a scratch buffer, `:write-buffer-close` if on a normal buffer. \
`:write-buffer-close-or-quit!` does `:quit!` if on a scratch buffer, `:write-buffer-close!` if on a normal buffer. \
`:buffer-close-or-quit` does `:quit` if on a scratch buffer, `:buffer-close` if on a normal buffer. \
`:buffer-close-or-quit!` does `:quit!` if on a scratch buffer, `:buffer-close!` if on a normal buffer.
...
`toggle_line_select` toggles between `trim_selections` and `extend_to_line_bounds`, based on your *primary* selection. \
If the entire selection is whitespace, `extend_to_line_bounds`. \
Otherwise, if at least "end" of the selection is a whitespace character, `trim_selections`. \
Otherwise, `extend_to_line_bounds`.
