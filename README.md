...
Also, the messages / commandline is merged with the statusline. \
...
Press <kbd>Space</kbd> or <kbd>'</kbd> to toggle between get / set. \
You can delete a register by first pressing <kbd>Delete</kbd>, and then the key that you want to delete. \
...
If you got into the `del` mode and go “nevermind”, you can either press <kbd>Escape</kbd> to cancel the harp action, or press <kbd>Space</kbd> or <kbd>'</kbd> to go back into `get` mode.

If you want to delete *all* of the registers, you can press <kbd>Shift+Delete</kbd>. \
Note: this hotkey works regardless of the mode you're in (get / set / del). \
And it also closes the whichkey ui afterwards.
...
In the whichkey ui, you get a line column + path column.
...
If you use `get` in insert mode, you immediately <kbd>ctrl+v</kbd> the result (`collapse_selection` + `paste_before` actions).
...
Press <kbd>k</kbd>¹ to make the harp relative to your current working directory. \
Press <kbd>l</kbd>¹ to make it relative to the current buffer. \
Press <kbd>;</kbd> to make it relative to the current buffer's filetype (run `:lang` to check the filetype of the current buffer). \
Press <kbd>j</kbd>¹ to go back to global relativity.
...
# I'm sorry
¹ for using letters here instead of the special characters as was in the blog post. \
After using harps for a while, I realized that easy accessibility of relativities is CRUCIAL, and pressing <kbd>,</kbd>, <kbd>.</kbd> all the time ended up being quite uncomfortable. \
I'm open to making these configurable actually, but I might have zero users aside from me so it's not a priority; please tell me in an issue if this thingy matters to you.
