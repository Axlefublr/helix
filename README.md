...
<kbd>alt+,</kbd> moves you to the start of the line and <kbd>alt+.</kbd> to the end, like <kbd>ctrl+a</kbd> and <kbd>ctrl+e</kbd> also do.

<kbd>alt+;</kbd> pastes the contents of your `default-yank-register`. A shortcut to <kbd>ctrl+r \<register\></kbd>, basically.

<kbd>ctrl+p</kbd> and <kbd>ctrl+n</kbd> go through history only if the input is empty. otherwise, they cycle through suggestions (which you can still cycle with <kbd>shift+tab</kbd> / <kbd>tab</kbd>)

### New command aliases

These are *added* aliases; I don't remove other aliases that already exist.

|Full command        |New alias(es)|
|--------------------|-------------|
|`:run-shell-command`|`:s`         |
|`:echo`             |`:c`         |
