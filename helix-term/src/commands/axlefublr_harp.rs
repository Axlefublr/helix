...
use crate::{
    commands::{
        self, command_line_doc, complete_command_line, execute_command_line, paste_impl, Context,
        Paste, PasteType,
    },
    compositor::{self},
...
impl Reciprocation {
    fn toggle(self) -> Self {
        match self {
            ...
            Self::Alternative => Self::Get,
        }
    }

    fn get_ish(&self) -> bool {
        self == &Self::Get || self == &Self::Alternative
    }
}
...
fn harp_resolve(
    ...
    cx.on_next_key(move |cx, event| {
        ...
        match &pressed[..] {
            ...
            the if the == hotkeys.alt => harp_resolve(
                cx,
                prompt,
                relativity,
                hotkeys,
                Reciprocation::Alternative,
                section,
                connection,
                width_figurouter,
                transformer,
            ...
            other => {
                if reciprocation == Reciprocation::Delete {
                    connection.section_mut(final_section).shift_remove(other);
                    if let Err(err) = connection.clone().save() {
                        cx.editor.set_error(err.to_string());
                    };
                    harp_resolve(
...
pub fn harp_file(cx: &mut Context) {
    ...
    harp_resolve(
        ...
        |cx, connection, reciprocation, section, register, relativity| {
            if reciprocation.get_ish() {
                ...
                let buffer_path = if relativity == HarpRelativity::Buffer {
                    let view = view!(cx.editor);
                    if let Some(id) = view.docs_access_history.iter().last() {
                        doc!(cx.editor, &id)
                            .path()
                ...
                harp_replace(cx, connection, section, register, vec![buffer_path])
            }
        },
    )
}
...
pub fn harp_relative_file(cx: &mut Context) {
    ...
    harp_resolve(
        ...
        |_| 0,
        |values, _, _| happy_family(values.first()).map(|the| the.display().to_string()),
        |cx, connection, reciprocation, section, register, _| {
            if reciprocation.get_ish() {
                harp_get_one(cx, &connection, &section, &register, |cx, value| {
                    cx.editor
                        .open(PathBuf::from(value).as_ref(), Action::Replace)
        ...
        |cx, connection, reciprocation, section, register, _| {
            if reciprocation.get_ish() {
                harp_get_one(cx, &connection, &section, &register, |cx, value| {
                    ...
                    crate::commands::save_selection(cx);
        ...
        |cx, connection, reciprocation, section, register, _| {
            if reciprocation.get_ish() {
                harp_get_one(cx, &connection, &section, &register, |cx, value| {
                    ...
                    if reciprocation == Reciprocation::Get {
                        match cx.editor.registers.first(':', cx.editor) {
                            Some(last_reg) => {
                                // history is not uniqued, up arrowing quickly becomes hell
                                if last_reg.trim() != value {
                                    let _ = cx.editor.registers.push(':', value.to_owned());
                                }
                            }
                            None => {
                        ...
                        // this is almost bar for bar the ↓ code
                        // helix-term/src/commands/typed.rs#command_mode
                        ...
                        .with_line(format!("{value} "), cx.editor);
