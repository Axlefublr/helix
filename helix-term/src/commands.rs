...
fn local_search_markdown_section(cx: &mut Context, file_contents: String) {
    ...
    let file_results: Vec<AlmostFileResult> = file_contents
        .lines()
        .enumerate()
        .filter_map(|(line_num, line)| {
            ...
            max_lengths[level - 1] = max_lengths[level - 1].max(title.len());
            ...
            let result = AlmostFileResult { line_num, parts };
            Some(result)
        })
        .collect();
                            ...
                            new_selection.clone().transform(|range| {
                                Range::new(
                                    ...
                                    if iteration == 1 {
                                        ...
                                        let from = range.from();
                                        if from == graphemes::prev_grapheme_boundary(text, from) {
                                            from
                                        } else if objtype == textobject::TextObject::Inside {
