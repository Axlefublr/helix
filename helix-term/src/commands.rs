...
fn local_search_markdown_section(cx: &mut Context, file_contents: String) {
    ...
    let file_results: Vec<FileResult> = file_contents
        ...
        .filter_map(|(line_num, line)| {
            let line = line.trim();
            if line.is_empty() || line.starts_with("#").not() {
                return None;
            }
            if let Some(title) = line.strip_prefix("# ") {
                ...
                if heading_stack.len() >= 2 {
                    heading_stack.drain(1..);
                }
                heading_stack.push(title.to_owned());
            } else if let Some(title) = line.strip_prefix("### ") {
                if heading_stack.len() >= 3 {
                    heading_stack.drain(2..);
                }
                heading_stack.push(title.to_owned());
            } else if let Some(title) = line.strip_prefix("#### ") {
                if heading_stack.len() >= 4 {
                    heading_stack.drain(3..);
                }
                heading_stack.push(title.to_owned());
            } else if let Some(title) = line.strip_prefix("##### ") {
                if heading_stack.len() >= 5 {
                    heading_stack.drain(4..);
                }
                heading_stack.push(title.to_owned());
            } else if let Some(title) = line.strip_prefix("###### ") {
                if heading_stack.len() >= 6 {
                    heading_stack.drain(5..);
                }
                heading_stack.push(title.to_owned());
            } else {
                return None;
